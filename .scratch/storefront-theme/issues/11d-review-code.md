...
| Escaping | Every `echo` of a variable traces to an escaped value: `esc_*`, `wp_kses_post` for `wc_price()` and cells, `wp_get_attachment_image()` output, `optimum_lift_icon()`, `optimum_lift_badge_html`-style builders that escape inside, and fixed tag names (`$tag`, `$heading`, `$tile_tag`). `optimum_lift_replace_tokens()` and `optimum_lift_heading_html()` escape before adding markup, and nothing wraps their output in `esc_*` again (grep finds none) | Yes | — |
